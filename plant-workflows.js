...
export function newPlant(){
  ...
  dialog('إضافة نبتة ومخزونها',content,async f=>{
    const p={id,code:'TP-'+id.slice(0,6).toUpperCase(),name:value(f,'name'),genus:readGenus(f),internalNotes:value(f,'notes'),publicDescription:'',image:'',published:f.has('sell'),archived:false,revision:0,variants:[{id:vid,type:value(f,'type'),qty:0,reserved:0,cost:0,price:amount(f,'price'),sell:f.has('sell')}]};
...
export function addProduction(p){
  ...
  const name=field('اسم الصنف الجديد','type','text','كورمة',{required:'',maxlength:'100'}),price=number('سعر بيع القطعة الواحدة (ر.س)','price'),sell=check('متاح للبيع — عرض النبتة في المتجر','sell',true);
  const content=el('div',{class:'form-grid'},el('p',{class:'hint full'},p.name+' — أضف عدد القطع المنتجة، وليس إجمالي المخزون. النبتة الأم تبقى كما هي.'),choice,name,quantity(),price,sell,details(number('تكلفة جديدة مدفوعة للقطعة (ر.س)','cost'),field('تاريخ الإنتاج','date','date',D.localDate(),{required:''})),el('p',{class:'hint full'},'اترك التكلفة صفراً إن لم تدفع مبلغاً جديداً. لا تكرر تكلفة مستلزمات مسجلة سابقاً. عند اختيار «متاح للبيع» تُنشر النبتة ويظهر الصنف بعد الحفظ.'));
