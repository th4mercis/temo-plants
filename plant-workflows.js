...
export function newPlant(){
  ...
  dialog('إضافة نبتة ومخزونها',content,async f=>{
    const p={id,code:'TP-'+id.slice(0,6).toUpperCase(),name:value(f,'name'),genus:value(f,'genus'),internalNotes:value(f,'notes'),publicDescription:'',image:'',published:false,archived:false,revision:0,variants:[{id:vid,type:value(f,'type'),qty:0,reserved:0,cost:0,price:amount(f,'price'),sell:f.has('sell')}]};
...
export function addProduction(p){
  ...
  const choice=select('أضف الإنتاج إلى','variant',[['new','صنف جديد: كورمة أو كتنج'],...p.variants.map(v=>[v.id,v.type])]);
  ...
  choice.querySelector('select').addEventListener('change',e=>{const v=p.variants.find(v=>v.id===e.target.value);name.hidden=!!v;name.querySelector('input').disabled=!!v;price.querySelector('input').value=((v?.price||0)/100).toFixed(2);sell.querySelector('input').checked=v?!!v.sell:true;});
